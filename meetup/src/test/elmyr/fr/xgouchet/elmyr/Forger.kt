...
open class Forger {
    ...
    fun aLinuxPath(absolute: Boolean? = null): String {
        ...
        val ancestorRoot = Array(aTinyInt()) { "src/main" }.joinToString(UNIX_SEP.toString()) { it }
        val roots = if (isAbsolute) LINUX_ROOTS else listOf(".", "src/main", ancestorRoot)
    ...
    fun aWindowsPath(absolute: Boolean? = null): String {
        val isAbsolute = absolute ?: aBool()
        val ancestorRoot = Array(aTinyInt()) { "src/main" }.joinToString(WINDOWS_SEP.toString()) { it }
        val roots = if (isAbsolute) WINDOWS_ROOTS else listOf(".", "src/main", ancestorRoot)
    ...
    fun aMacOsPath(absolute: Boolean? = null): String {
        val isAbsolute = absolute ?: aBool()
        val ancestorRoot = Array(aTinyInt()) { "src/main" }.joinToString(UNIX_SEP.toString()) { it }
        val roots = if (isAbsolute) MAC_ROOTS else listOf(".", "src/main", ancestorRoot)
        val forbiddenChars = arrayOf(0.toChar(), UNIX_SEP).toCharArray()
        return aPath(UNIX_SEP.toString(), roots, MAX_PATH_SIZE, MAX_FILENAME_SIZE, forbiddenChars)
    }
